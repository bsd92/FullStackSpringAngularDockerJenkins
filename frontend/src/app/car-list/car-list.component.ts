...
export class CarListComponent implements OnInit {
  ...
  constructor(private carsService: CarsService, private router:Router, private authService:AuthService, private http:HttpClient){}
  ...
  statusList = ['EN_ATTENTE', 'EN_COURS', 'TERMINE', 'DELAI_PROLONGE'];

onStatusChange(car: any, newStatus: string) {
  car.status = newStatus;
  //this.updateCarStatus(car); // Tu peux appeler un service ici
}

updateCarStatut(car: any) {
  this.http.put(`http://localhost:8080/garage/cars/${car.immatriculation}/status`, car.status).subscribe(() => {
    alert(`Voiture avec immatriculation ${car.immatriculation} mis à jour !`);
  });
}
